...
5. Framework compliance: a card per benchmark (MCSB v2, CIS Azure 6.0.0, WAF, ALZ) and for DORA (a JSolve crosswalk to the DORA and RTS articles with a technical Azure side), with version, publisher and source, the derived frameworks (NIST SP 800-53, PCI DSS, ISO 27001, CIS Controls, NIST CSF, SOC 2), and a control table per framework. Controls link to their source documentation where one exists; derived controls show the MCSB controls they come from.
...
7. Scope and sources: assessment scope, result definitions and every framework's version, publisher, access terms and source.
...
Every chart has a table view. Status is always shown with an icon and a label, never by color alone. Analyses made before schema 2 still render, without per-control sources.
